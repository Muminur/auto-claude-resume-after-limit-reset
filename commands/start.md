...
**macOS/Linux:**
```bash
node ~/.claude/plugins/cache/auto-claude-resume/auto-resume/1.19.0/auto-resume-daemon.js start
```

**Windows (PowerShell):**
```powershell
node "$env:USERPROFILE\.claude\plugins\cache\auto-claude-resume\auto-resume\1.19.0\auto-resume-daemon.js" start
```

**Windows (CMD/Git Bash):**
```cmd
node "%USERPROFILE%\.claude\plugins\cache\auto-claude-resume\auto-resume\1.19.0\auto-resume-daemon.js" start
```

> **Note:** When Claude Code executes these commands via Bash tool, use the direct Windows path:
> `node "C:\Users\YOUR_USERNAME\.claude\plugins\cache\auto-claude-resume\auto-resume\1.19.0\auto-resume-daemon.js" start`
...
**macOS/Linux:**
```bash
nohup node ~/.claude/plugins/cache/auto-claude-resume/auto-resume/1.19.0/auto-resume-daemon.js start > /dev/null 2>&1 &
```

**Windows (PowerShell):**
```powershell
Start-Process -NoNewWindow node -ArgumentList "$env:USERPROFILE\.claude\plugins\cache\auto-claude-resume\auto-resume\1.19.0\auto-resume-daemon.js", "start"
```
